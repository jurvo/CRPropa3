...
AdvectionFieldFromList::AdvectionFieldFromList(std::string path, double scale) : scale(scale) {
    loadData(path);
}
...
void AdvectionFieldFromList::loadData(std::string path) {
   ...
   std::istream *in;
    ...
    while (std::getline(*in, line))
    {
        ...
        velocity.push_back(v * scale);
    }
    infile.close();
}

Vector3d AdvectionFieldFromList::getField(const Vector3d &pos) const {
