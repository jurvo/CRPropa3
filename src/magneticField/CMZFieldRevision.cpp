...
// Transformation from cartesian coordinates to the phi component of polar/zylindrical coordinates
double CMZField::PHI(double x, double y) const{
    double pi=M_PI;
    double val=0;
    if (x>0)
    {
...
Vector3d CMZField::MCField(const Vector3d& pos) const {//Field in molecular clouds
	double pi=M_PI;
    double m=1;
    Vector3d b(0.);
    double eta=0.01;
...
Vector3d CMZField::ICField(const Vector3d& pos) const {//Field in intercloud medium--> poloidal field
    Vector3d b(0.);
    double pi= M_PI;
    ...
    //  
Vector3d CMZField::NTFField(const Vector3d& pos) const {//Field in the non-thermal filaments--> predominantly poloidal field
    Vector3d b(0.);
    double pi= M_PI;
    ...
    //  
Vector3d CMZField::RadioArc(const Vector3d& pos) const {//Field in the non-thermal filaments--> predominantly poloidal field
    Vector3d b(0.);
...
Vector3d CMZField::getField(const Vector3d& pos) const{
    ...
    if (-5.*pc<pos.y<50.*pc and -30.*pc<pos.x<30.*pc)
    {
        B=RadioArc(pos);
    }
