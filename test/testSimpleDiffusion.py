# coding=utf-8
# copied testDiffusionSDE.py and just replaced DiffusionSDE with SimpleDiffusion
import sys
...
class DiffusionOneDirection(unittest.TestCase):
    ...
    Dif = crpropa.SimpleDiffusion(BField, precision, minStep, maxStep, epsilon)
    DifAdv = crpropa.SimpleDiffusion(BField, precision, minStep, maxStep, epsilon)
    ...
    def test_AdvectivePropagation(self):
        ...
        Dif = crpropa.SimpleDiffusion(BField, AdvField, precision, minStep, maxStep, epsilon)
        c = crpropa.Candidate()
        c.current.setId(crpropa.nucleusId(1,1))
        c.current.setEnergy(10*TeV)
        c.current.setDirection(crpropa.Vector3d(1,0,0))
    ...
    def test_FullTransport(self):
        ...
        DifAdv = crpropa.SimpleDiffusion(BField, AdvField, precision, minStep, maxStep, epsilon)
